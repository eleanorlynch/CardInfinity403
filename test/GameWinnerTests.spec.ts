...
describe("GameWinner", function () {
    describe("#checkMostSuit()", function () {
        it("should return the player with the most cards of one suit if there is no tie", function() {
            ...
            game.setPlayerHand([new Card("diamonds", 2), new Card("hearts", 2), new Card("diamonds", 2), new Card("hearts", 2), new Card("hearts", 2)], 1);
            game.setPlayerHand([new Card("diamonds", 2), new Card("spades", 2), new Card("diamonds", 2), new Card("hearts", 2), new Card("hearts", 2)], 0);
            game.setRound(3);
            const winner = new GameWinner;
            assert.deepStrictEqual(winner.checkMostSuit(game), {tie: false, winner: 1});
        });
        it("should return the players with the most card of one suit if there is a tie", function() {
            ...
            game.setPlayerHand([new Card("diamonds", 2), new Card("diamonds", 2), new Card("hearts", 2), new Card("hearts", 2), new Card("hearts", 2)], 1);
            game.setRound(3);
            const winner = new GameWinner;
